...
export async function callEntryFunc(
  ...
  const key: string | undefined = NETWORKS[network]['aptos'].key;
  if (key === undefined) {
    throw new Error('No key for aptos');
  }

  const rpc: string | undefined = NETWORKS[network]['aptos'].rpc;
  if (rpc === undefined) {
    throw new Error('No rpc for aptos');
  }

  const accountFrom = new AptosAccount(new Uint8Array(Buffer.from(key, 'hex')));
  const client: AptosClient = new AptosClient(rpc);
